...
/**
 * @interface task_snapshot
 * @description 1 回のスナップショット記録。タスクの変更前状態を保持する。
 * uuid フィールドにより、move_task 後もスナップショットを追跡可能。
 */
export interface task_snapshot {
    uuid:      string;  // 不変 UUID（move_task を経ても変わらない）
    task_id:   string;  // 記録時点の Google Tasks タスク ID（変わりうる）
    list_id:   string;  // 記録時点のリスト ID
...
export const snapshot = {
     ...
     */
    save(uuid: string, task_id: string, list_id: string, state: Record<string, unknown>): void {
        fs.mkdirSync(SNAPSHOT_DIR, { recursive: true });

        const entry: task_snapshot = {
            uuid,
            task_id,
            list_id,
        ...
        let history: task_snapshot[] = [];
        if (fs.existsSync(file)) {
            history = JSON.parse(fs.readFileSync(file, 'utf8')) as task_snapshot[];
        }
        history.push(entry);
     ...
     */
    restore(uuid: string): task_snapshot | null {
        const file = path.join(SNAPSHOT_DIR, `${uuid}.json`);
        if (!fs.existsSync(file)) return null;

        const history = JSON.parse(fs.readFileSync(file, 'utf8')) as task_snapshot[];
     ...
     */
    list_snapshots(uuid?: string): task_snapshot[] {
        if (!fs.existsSync(SNAPSHOT_DIR)) return [];

        if (uuid) {
            const file = path.join(SNAPSHOT_DIR, `${uuid}.json`);
            if (!fs.existsSync(file)) return [];
            return JSON.parse(fs.readFileSync(file, 'utf8')) as task_snapshot[];
        }

        return fs.readdirSync(SNAPSHOT_DIR)
            .filter(f => f.endsWith('.json'))
            .flatMap(f =>
                JSON.parse(fs.readFileSync(path.join(SNAPSHOT_DIR, f), 'utf8')) as task_snapshot[]
            );
    },
};
