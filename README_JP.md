...
Gentask は **Google Tasks** と **Google Calendar** を **Gemini 2.0 Flash (Vertex AI)** と GenKit を用いて統合する CLI ツールです。週刊漫画連載の制作サイクルを自動的・知的に管理します。
...
作家は Google Calendar という「自由なキャンバス」で作業時間を動かし、メモを書く。Gentask（AI）は、その自由な振る舞いの背後にある「18sp モデルとの差分」を計算し、Google Tasks という「厳格な帳簿」を無言で更新し続ける。
...
タスクは 4 つの実行モードに分類され、それぞれ専用の Google Tasks のタスクリストに対応します：
...
│  │ tasks.ts    │────► タスクリスト / タスク作成         │
│  │ デプロイ    │────► カレンダーイベントと連携          │
│  └──────┬──────┘                                        │
│         │                                                │
│         ▼                                                │
│  ┌─────────────┐     Google Calendar API                 │
│  │ calendar.ts │────► カレンダーイベント読み取り         │
│  └──────┬──────┘                                        │
│         │                                                │
│         ▼                                                │
│  ┌─────────────┐     AI（Gemini 2.0 Flash / Vertex AI） │
...
### 1. AI タスク生成（`gen`）
題材（例：「第42話 最終決戦」）を入力すると、Gemini 2.0 Flash（Vertex AI）を GenKit 経由で利用して P/T/C/A の構造化タスク一覧を生成します。タスクは即座に Google Tasks の適切なタスクリストへデプロイされ、Google Calendar イベントと連携されます。

### 2. インテリジェント AI シンクロナイザー（`sync`）
Google Calendar のイベントを読み取り、AI が自由記述のメモを構造化された進捗シグナルとして解釈します：

| Google Calendar でのユーザー行動 | AI の解釈 | Google Tasks への反映 |
|---|---|---|
...
4. **スケジュール** — 昇格したタスクを月〜金の Google Calendar に自動配置
5. **新規生成** — AI が次々回話数のプロットタスクを「来週分」バケットに生成

### 5. 双方向 Open Extensions
すべての Google Calendar イベントと Google Tasks に相互参照が埋め込まれます：
...
```sh
# 題材からタスクを生成して Google Tasks にデプロイ（開発環境）
...
- 各モードに 3 バケット（今週分 / 来週分 / 完了）のタスクリスト（Google Tasks）を作成
- 正しいタスクリストにタスクをデプロイ
- 連携する Google Calendar イベントを作成
- タスクとイベント間の相互参照メタデータを保存

### AI シンク（Calendar から Tasks に進捗反映）

```sh
# Google Calendar イベントを読み取り Google Tasks に進捗反映（開発環境）
npm run sync:dev
...
月〜日
  ...
  │
  │  npm run sync:dev  ← いつでも実行して進捗を Google Tasks に反映
  │
日曜 21:00
  │  投稿完了 ✅
  │
