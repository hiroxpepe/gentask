...
CTASK モードの「今週分」リストに「投稿」という文字を含むタスクが存在し、かつ `status: completed` であることを確認します。未完了の場合はスライド処理を中断します。
...
昇格したタスクに対して Google Calendar にイベントを作成します。タスクのタイトルに応じてスケジュール配置ルールを適用します：

| タイトル | 配置ルール |
| :--- | :--- |
| 「プロット」を含む | 水曜 14:00（1h）、木曜 14:00（1h）の 2 イベント |
| 「ネーム」を含む | 金曜 14:00（1h）の 1 イベント |
| それ以外 | 翌月曜 09:00 から 30 分ブロックで順次配置 |
...
* **状態スナップショット・エンジン:** AIの誤判定や操作ミスが発生した場合、システムは常に「一つ前の状態」へロールバック可能な安全装置を提供します。スナップショットは `~/.gentask/snapshots/{taskId}.json` に保存されます。
