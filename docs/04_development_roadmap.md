...
│   ├── google.test.ts
│   └── google-container-manager.ts  # Google Tasks 12リスト管理
│
├── docs/                         # プロジェクトドキュメント
...
task_flow（Genkit + Gemini）
  ...
  │
  ▼ 各タスクについて:
GoogleContainerManager.get_container(mode, auth)
...
tasks.tasks.insert({ tasklist: listId, requestBody: { title, notes: description } })
  ...
  │
  ▼
calendar.events.insert({ calendarId, requestBody: { extendedProperties.private: { gentask_taskId, gentask_listId } } })
  → eventId を取得
  │
  ▼
...
入力: なし（自動的にカレンダーを参照）
  │
  ▼
calendar.events.list({ privateExtendedProperty: "gentask_taskId", timeMin: 2週間前 })
  → gentask管理イベントのみ取得
  │
  ▼ 各イベントについて:
tasks.tasks.get({ tasklist: listId, task: taskId })
  → currentStatus（0 or 100）を取得
...
sync_flow（Genkit + Gemini）
  ...
  │
  ▼
GoogleSyncService.apply_actions(actions, list_map)
...
archive_current_week(container, auth, mode)
  [CTASKのみ] 投稿タスク完了チェック → false ならスキップ
...
schedule_promoted_tasks(promoted, auth)
  各タスクを PLANNING_SCHEDULE に基づきカレンダーへ配置
  calendar.events.insert → eventId 取得
  tasks.tasks.update でタスクノートに双方向リンク埋め込み
  │
  ▼ mode === 'PTASK' の場合のみ:
generate_next_plot(container, episodeHint, auth)
...
| `bin/sync.test.ts` | `GoogleSyncService.apply_actions` | ~8 |
...
詳細な改修手順は `docs/develop_plan.md` を参照してください。
